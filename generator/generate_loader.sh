  ...
  for hdr in *.h; do
    ...
    if [[ ${hdr/d3d/} == ${hdr} && ${hdr/dx/} == ${hdr} && ${hdr/intel/} == ${hdr} ]]; then
      ...
      if [[ ${hdr} != "opencl.h" ]]; then
        includes="${includes}#include \"${CL_SUBDIR}/${hdr}\""
      fi
    fi
  done
...
EOF
  ...
  hdr="${include_out_dir}"/cl.hpp
  if [[ -f "${hdr}" ]]; then
    echo "// cl.hpp Processed by OpenCL-Loader Generator $(date)" > "${hdr}.fixed"
